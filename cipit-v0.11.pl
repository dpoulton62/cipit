...
#
# Check out some good press we received for this project
# http://www.networkworld.com/community/blog/cisco-doesnt-contribute-nearly-enough-open-so
# 
...
#		   -IO::Socket::INET to test if TCP port 80 is open
...
#	-Commandline option for multiple iterations through the SYN/ACK discovery
...
# Create the $opt_outputfile and begin checking content
...
BIG:	foreach (@PhoneIPs) {
		...
		$xml_url = "http://$_" . '/' . "DeviceInformationX";				# XML url for Main Device Information Page
		$content = get $xml_url;
...
sub CIDR_to_IPs {
	...
	for ($count = $opt_repeat; $count >= 1; $count--) {
		...
		while (my ($host,$rtt,$ip) = $p->ack) {
			...
			if (IO::Socket::INET->new(PeerAddr=>$host,PeerPort=>80,proto=>'tcp',Timeout=>$opt_timeout)) {
				# Normally just be quite and add output to CSV
				if ($opt_v) {
					print "Ack:(" , $host , ":80) is open.\n\n";
				}
			}
